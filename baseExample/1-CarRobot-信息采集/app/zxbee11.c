...
static char D0 = 0xF0;                                          // 主动上报开关，默认全开
static unsigned int V0 = 30;                                    // 主动上报时间间隔，单位秒,0不主动上报
...
{
  int ret = -1;							
  if (memcmp(ptag, "D1", 2) == 0) {				// 读取D1状态
    if (pval[0] == '?') {
      ret = sprintf(obuf, "D1=%d", D1);
    }
  }
  if (memcmp(ptag, "CD1", 3) == 0) {				// 关闭D1
     int v = atoi(pval);
       D1 &= ~v;
       //sensor_control(D1);
     }
  
  if (memcmp(ptag, "OD1", 3) == 0) {				// 打开D1
     int v = atoi(pval);  
       D1 |= v;
       //sensor_control(D1);
     }
  ...
  if (memcmp(ptag, "D0", 2) == 0) {				// 读取D0状态
    if (pval[0] == '?') {
      ret = sprintf(obuf, "D0=%d", D0);
    }
  }
  if (memcmp(ptag, "CD0", 3) == 0) {				// 关闭D0
     int v = atoi(pval);
     if (v > 0) {
       D0 &= ~v;
     }
  }
  if (memcmp(ptag, "OD0", 3) == 0) {				// 打开D0
     int v = atoi(pval);
     if (v > 0) {
       D0 |= v;
     }
  }
  if (memcmp(ptag, "V0", 2) == 0) {				// 读取V0数值
    if (pval[0] == '?') {
  ...
  if (memcmp(ptag, "A4", 2) == 0) {				// 读取A4数值
    if (pval[0] == '?') {
      ret = sprintf(obuf, "A4=%.1f", A4);
    }
  }
  if (memcmp(ptag, "A5", 2) == 0) {				// 读取A5数值
    if (pval[0] == '?') {
      ret = sprintf(obuf, "A5=%.1f", A5);
    }
  }
  if (memcmp(ptag, "A6", 2) == 0) {				// 读取A6数值
    if (pval[0] == '?') {
      ret = sprintf(obuf, "A6=%.1f", A6);
    }
  }
   if (memcmp(ptag, "A7", 2) == 0) {				// 读取A7数值
    if (pval[0] == '?') {
