...
void LCDDrawAsciiDot12x24_1(int x, int y, char ch, int color, int bc)
{
    ...
    for (int i=0; i<3; i++) {
        for (int j=0; j<12; j++) {
            ...
            for (int k=0; k<8; k++) {
                if (dot&1)
                    LCDDrawPixel(x+j, y+(i*8)+k, color);
                else
                    LCDDrawPixel(x+j, y+(i*8)+k, bc);
                dot >>= 1;
            }
        }
    }
...
void LCD_ShowPic(int x,int y,int length,int height,unsigned char *picture)
{
    unsigned long i;
    LCDSetWindow(x,x+length-1,y,y+height-1);
    LCDSetCursor(x, y);
    ILI93xx_REG = CMD_WR_RAM;                                       //显示命令
    for(i=0;i<length*height;i++)
        ILI93xx_DAT = ((picture[i*2+1]<<8)+picture[i*2]);
#if SCREEN_ORIENTATION_LANDSCAPE  
    LCDSetWindow(0, 320, 0, 240);                               //设置窗口为整个屏幕 
#else
    LCDSetWindow(0, 240, 0, 320);
    ...
    for(b=0;b<1;b++)
    {
        ...
        for(i=0;i<16;i++) 
        {
            ...
            for(j=0;j<8;j++) 
            {
                ...
                if ( (tmp_char << j) & 0x80)
                {
                    LCDDrawPixel(x+j+8,y+i,Color);
                }
                else
                {
                    LCDDrawPixel(x+j+8,y+i,BColor);
                }
            }		
        }
    }  
...
void lcdShowFont16(uint16 x,uint16 y,char* p,uint16 color,uint16 Bcolor)
{
    ...
    while(*temp!='\0')
    {
        ...
        {
            LCDDrawAsciiDot8x16_1(x1, y, *temp, color, Bcolor);
            temp ++;
            x1 += 8;
        }
    }
